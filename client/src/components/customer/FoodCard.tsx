...
export default function FoodCard({ menu, compact = false }: FoodCardProps) {
    ...
    return <article className={["group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-[0_8px_24px_rgba(41,37,36,0.07)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_14px_30px_rgba(41,37,36,0.12)]", compact ? "shadow-[0_5px_16px_rgba(41,37,36,0.06)]" : ""].join(" ")}>
        <div className={["relative overflow-hidden rounded-xl bg-stone-100", compact ? "m-2.5 aspect-[4/3]" : "m-3 aspect-[4/3]"].join(" ")}>
        ...
        <div className={["flex flex-1 flex-col", compact ? "px-4 pb-4 pt-0.5" : "px-5 pb-5 pt-1"].join(" ")}>
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-baseline gap-x-2 gap-y-1"><h3 className={["font-display font-bold leading-tight text-stone-900", compact ? "text-lg" : "text-xl"].join(" ")}>{menu.food.name}</h3>{menu.food.reviewCount > 0 && menu.food.averageRating !== null ? <span className="inline-flex items-center gap-1 whitespace-nowrap text-sm font-semibold text-stone-600"><StarRating rating={menu.food.averageRating} className="text-sm" />{menu.food.averageRating.toFixed(1)} ({menu.food.reviewCount})</span> : <span className="text-xs font-medium text-stone-400">No reviews yet</span>}</div></div><span className={["shrink-0 font-mono font-bold text-orange-600", compact ? "text-base" : "text-lg"].join(" ")}>₱{Number(menu.food.price).toFixed(2)}</span></div>
            {!compact && <p className="mt-3 line-clamp-2 min-h-10 text-sm leading-5 text-stone-500">{menu.food.description}</p>}
            <div className={["flex items-center justify-between gap-3 border-t border-stone-100", compact ? "mt-4 pt-3" : "mt-5 pt-4"].join(" ")}>
